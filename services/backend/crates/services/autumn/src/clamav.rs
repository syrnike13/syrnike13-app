...
pub async fn init() {
    ...
    if !config.files.clamd_host.is_empty() {
        ...
        loop {
            let clamd_available =
                match syrnike_clamav_client::ping_tcp(config.files.clamd_host.clone()) {
...
pub async fn is_malware(buf: &[u8]) -> Result<bool> {
    ...
    if config.files.clamd_host.is_empty() {
        ...
        let scan_response = report_internal_error!(syrnike_clamav_client::scan_buffer_tcp(
            buf,
            config.files.clamd_host,
            None
        ))?;

        report_internal_error!(syrnike_clamav_client::clean(&scan_response)).map(|v| !v)
    }
}
