...
use syrnike_database::{
    ...
    voice::{
        ...
        refresh_voice_credentials,
        remove_user_from_voice_channel, set_user_voice_join_intent, update_client_voice_flags,
...
pub async fn handle_voice_state_update(
    ...
    if channel_id.is_none() {
        for channel in get_user_voice_channels(&user.id).await? {
            remove_user_from_voice_channel(voice_client, &channel, &user.id).await?;
        }
