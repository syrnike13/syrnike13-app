...
use syrnike_config::{config, Settings};
...
async fn main() {
    ...
    if let Some(client) = match &db {
        syrnike_database::Database::Reference(_) => None,
        syrnike_database::Database::MongoDb(mongo) => Some(mongo),
    } {
        authifier =
            authifier::Database::MongoDb(authifier::database::MongoDb(client.database("syrnike")));
    } else {
    ...
    F: Consumer,
{
    ...
    channel
        ...
        .await
        .expect(
            "This probably means the syrnike.notifications exchange does not exist in rabbitmq!",
        );
