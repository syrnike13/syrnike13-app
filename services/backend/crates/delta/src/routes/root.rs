...
impl UserLimits {
    fn from_feature_limits(fl: syrnike_config::FeaturesLimits) -> UserLimits {
        UserLimits {
...
#[derive(Serialize, JsonSchema, Debug)]
pub struct SyrnikeConfig {
    /// syrnike13 API Version
    pub syrnike: String,
    /// Features enabled on this syrnike13 node
    pub features: SyrnikeFeatures,
    /// WebSocket URL
    pub ws: String,
...
#[openapi(tag = "Core")]
#[get("/")]
pub async fn root() -> Result<Json<SyrnikeConfig>> {
    let config = config().await;

    Ok(Json(SyrnikeConfig {
        syrnike: env!("CARGO_PKG_VERSION").to_string(),
