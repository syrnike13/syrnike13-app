...
macro_rules! report_error {
    ( $expr: expr, $error: ident $( $tt:tt )? ) => {
        $expr
            ...
            .map_err(|_| ::syrnike_result::create_error!($error))
    };
}
...
macro_rules! report_internal_error {
    ( $expr: expr ) => {
        $expr
            ...
            .map_err(|_| ::syrnike_result::create_error!(InternalError))
    };
}
...
static CONFIG_SEARCH_PATHS: [&str; 3] = [
    ...
    "Syrnike.overrides.toml",
    // root directory, for Docker containers
    "/Syrnike.toml",
];

/// Path to search for test overrides
static TEST_OVERRIDE_PATH: &str = "Syrnike.test-overrides.toml";

/// Configuration builder
static CONFIG_BUILDER: Lazy<RwLock<Config>> = Lazy::new(|| {
    RwLock::new({
        let mut builder = Config::builder().add_source(File::from_str(
            include_str!("../Syrnike.toml"),
            FileFormat::Toml,
        ));

        if std::env::var("TEST_DB").is_ok() {
            builder = builder.add_source(File::from_str(
                include_str!("../Syrnike.test.toml"),
                FileFormat::Toml,
            ));
        ...
        builder = builder.add_source(Environment::with_prefix("SYRNIKE").separator("__"));
...
pub async fn config() -> Settings {
    ...
    if config.hosts.api.contains("https") && config.hosts.api.contains("syrnike13.ru") {
