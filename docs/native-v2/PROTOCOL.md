...
The descriptor is a drift detector rather than a complete IDL: Effect schemas, C++ structs, and N-API codecs are still maintained explicitly. Before any track intent gains values beyond `off`, the descriptor must grow typed fields/enums/unions and generate those representations and codecs, or be replaced by a full IDL.
