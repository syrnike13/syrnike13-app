...
| Room reconciliation | Engine plus `RoomOwner` | Engine control thread | Accepted room intent | Room off or shutdown | Non-blocking completion and generation fencing | No |
| LiveKit `Room` | `LiveKitRoomTransport` | One persistent transport worker | Accepted intent with valid lease | Disconnect before shutdown | SDK timeout and disconnect cancellation; process is final escalation | No |
