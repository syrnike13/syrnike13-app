...
void emitScreenSourceList(uintptr_t excluded_window_hwnd) {
  ...
  WindowEnumContext window_context{
      &sources,
      reinterpret_cast<HWND>(excluded_window_hwnd),
  };
  EnumWindows(enumWindowProc, reinterpret_cast<LPARAM>(&window_context));

  std::ostringstream json;
  json << "{\"type\":\"display_source_list\",\"sources\":[";
  for (size_t i = 0; i < sources.size(); ++i) {
    const auto& source = sources[i];
    if (i != 0) json << ",";
    json << "{\"id\":\"" << jsonEscape(source.id)
         << "\",\"name\":\"" << jsonEscape(source.name)
         << "\",\"type\":\"" << source.type
         << "\",\"thumbnailDataUrl\":"
         << (source.thumbnail_data_url.empty()
                 ? "null"
                 : "\"" + jsonEscape(source.thumbnail_data_url) + "\"")
         << ",\"appIconDataUrl\":"
         << (source.app_icon_data_url.empty()
                 ? "null"
                 : "\"" + jsonEscape(source.app_icon_data_url) + "\"")
         << ",\"processId\":" << source.process_id
         << ",\"processPath\":"
         << (source.process_path.empty()
                 ? "null"
                 : "\"" + jsonEscape(source.process_path) + "\"")
         << ",\"classification\":\"" << jsonEscape(source.classification) << "\""
         << ",\"audioAvailable\":" << (source.audio_available ? "true" : "false")
         << ",\"audioMode\":\"" << jsonEscape(source.audio_mode) << "\""
         << "}";
  }
  json << "]}";
  emit(json.str());
