...
export function useVoiceAudioInputDevices() {
  const useEngine = shouldUseDesktopMediaEngine()
  const browserDevices = useMediaDevices('audioinput')
  ...
  useEffect(() => {
    ...
    const stored = voicePreferenceStore.getPreferredAudioInputDevice()
    if (!stored) return
    ...
    if (reconciled) {
      voicePreferenceStore.setPreferredAudioInputDevice(reconciled)
      return
    }

    voicePreferenceStore.setPreferredAudioInputDevice(undefined)
