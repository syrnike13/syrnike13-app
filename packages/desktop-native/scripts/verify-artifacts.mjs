...
if (JSON.stringify(actualNames) !== JSON.stringify([...EXPECTED_FILES].sort())) {
  throw new Error(
...
if (
  ...
  JSON.stringify(manifest.capabilities) !== JSON.stringify(['hotkeys', 'overlay']) ||
  !Array.isArray(manifest.files)
) {
  throw new Error('Native artifact manifest has an unsupported shape')
}

const binaries = EXPECTED_FILES.filter((name) => name !== 'native-manifest.json')
const expectedHashes = new Map(
  manifest.files.map((entry) => [entry.name, entry.sha256]),
)
if (expectedHashes.size !== binaries.length || manifest.files.length !== binaries.length) {
...
console.info(`[desktop-native] verified hook-only artifact set at ${targetRoot}`)
