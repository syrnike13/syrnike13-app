...
const char* BackendName(VideoEncoderBackend backend) {
  switch (backend) {
    ...
    case VideoEncoderBackend::VideoToolbox:
      return "videotoolbox";
    case VideoEncoderBackend::WindowsD3D11Hardware:
      return "d3d11-h264";
  }
}
...
std::optional<VideoEncoderBackend> BackendFromFormat(
  ...
  if (it->second == BackendName(VideoEncoderBackend::WindowsD3D11Hardware)) {
    return VideoEncoderBackend::WindowsD3D11Hardware;
  }
...
bool IsSpecificHardwareBackend(VideoEncoderBackend backend) {
  return backend == VideoEncoderBackend::Nvenc ||
         ...
         backend == VideoEncoderBackend::WindowsD3D11Hardware;
}
...
void AddWindowsD3D11Factory(
    std::vector<VideoEncoderBackendFactory>& factories) {
#ifdef _WIN32
  auto factory = CreateWindowsD3D11HardwareH264EncoderFactory();
  if (factory) {
    AddBackendFactory(factories, VideoEncoderBackend::WindowsD3D11Hardware,
                      std::move(factory));
  }
#else
  (void)factories;
...
rust::Vec<VideoEncoderBackend> video_encoder_backend_list() {
  ...
  if (IsWindowsD3D11HardwareH264Supported()) {
    backends.push_back(VideoEncoderBackend::WindowsD3D11Hardware);
    has_hardware_backend = true;
  }
#endif
...
VideoEncoderFactory::InternalFactory::InternalFactory() {
  ...
  AddWindowsD3D11Factory(factories_);
...
VideoEncoderFactory::InternalFactory::QueryCodecSupport(
  ...
  if (requested_backend == VideoEncoderBackend::Software) {
    ...
    // An explicitly selected backend is a hard requirement. Reporting a
    // different implementation as supported here lets WebRTC negotiate a
    // codec that Create() must later reject and can trigger an implicit
    // software fallback in higher layers.
...
VideoEncoderFactory::InternalFactory::Create(
  ...
  if (requested_backend_unavailable) {
    RTC_LOG(LS_ERROR) << "Required video encoder backend "
