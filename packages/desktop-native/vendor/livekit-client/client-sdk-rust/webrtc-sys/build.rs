...
fn main() {
    ...
    if target_os == "windows" {
        builder.file("src/windows_d3d11_h264_encoder.cpp");
    }
    ...
    for (key, value) in webrtc_sys_build::webrtc_defines() {
        if target_os == "windows" && key == "NTDDI_VERSION" {
            // The prebuilt archive was produced with a newer Windows SDK and
            // names NTDDI_WIN11_GE, which the windows-2022 SDK cannot expand.
            builder.define("NTDDI_VERSION", "0x0A00000C");
            continue;
        }
