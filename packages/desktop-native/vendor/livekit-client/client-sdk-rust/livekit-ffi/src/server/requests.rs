...
fn on_capture_d3d11_video_frame(
    server: &'static FfiServer,
    push: proto::CaptureD3d11VideoFrameRequest,
) -> FfiResult<proto::CaptureD3d11VideoFrameResponse> {
    #[cfg(not(target_os = "windows"))]
    return Err(FfiError::InvalidRequest(
        "D3D11 video frames are supported only on Windows".into(),
    ));
...
pub fn handle_request(
    ...
    res.message = Some(match request {
        ...
        Request::CaptureD3d11VideoFrame(req) => on_capture_d3d11_video_frame(server, req)?.into(),
