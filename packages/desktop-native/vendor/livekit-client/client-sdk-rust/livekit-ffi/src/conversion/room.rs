...
fn video_encoder_from_proto(backend: Option<i32>) -> Option<VideoEncoderBackend> {
    match backend.and_then(|value| proto::VideoEncoderBackend::try_from(value).ok())? {
        ...
        proto::VideoEncoderBackend::EncoderBackendWindowsD3d11Hardware => {
            Some(VideoEncoderBackend::WindowsD3D11Hardware)
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn windows_d3d11_hardware_backend_is_exact() {
        let backend = video_encoder_from_proto(Some(
            proto::VideoEncoderBackend::EncoderBackendWindowsD3d11Hardware.into(),
        ));

        assert_eq!(backend, Some(VideoEncoderBackend::WindowsD3D11Hardware));
    }
