  ...
  explicit HooksRuntimeBinding(const Napi::CallbackInfo& info)
    ...
    if (active_runtime) {
      throw Napi::Error::New(
        info.Env(), "runtime_already_created: hooks runtime is singleton per utility process"
      );
    }
    auto sink = std::make_shared<NodeEventSink>(
      info.Env(), info[0].As<Napi::Function>(), "syrnike-hooks-events"
    );
...
Napi::Object getRuntimeInfo(const Napi::CallbackInfo& info) {
  ...
  auto capabilities = Napi::Array::New(info.Env(), 2);
  capabilities.Set(uint32_t{0}, "hotkeys");
  capabilities.Set(uint32_t{1}, "overlay");
...
Napi::Object initialize(Napi::Env env, Napi::Object exports) {
  ...
  exports.Set("createHooksRuntime", Napi::Function::New(env, createHooksRuntime));
...
NODE_API_MODULE(syrnike_hooks, initializeHooksAddon)
