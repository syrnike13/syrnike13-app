...
int main() {
  {
    auto mailbox = std::make_unique<ActorMailbox<4, 2>>();
    int released = 0;
  ...
  {
    auto mailbox = std::make_unique<ActorMailbox<2, 2>>();
    int released = 0;
    require(mailbox->tryPush(frame(
  ...
  {
    auto mailbox = std::make_unique<ActorMailbox<1, 1>>();
...

  {
    ...
    auto mailbox = std::make_unique<ActorMailbox<8, 2>>();
  ...
  {
    auto mailbox = std::make_unique<ActorMailbox<2, 1>>();
    int released = 0;
    require(mailbox->tryPush(frame(
  ...
  {
    auto mailbox = std::make_unique<ActorMailbox<4, 4>>();
    int released = 0;
    require(mailbox->tryPush(frame(
  ...
  {
    // Keep the intentionally large full-capacity fixture off the test thread's
    // stack so its storage does not dominate process-path stack analysis.
    auto mailbox = std::make_unique<ActorMailbox<2, 64>>();
