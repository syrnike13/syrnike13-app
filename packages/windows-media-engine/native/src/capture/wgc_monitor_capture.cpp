...
CaptureFailureKind failureKind(HRESULT result) noexcept {
  if (result == E_ACCESSDENIED) return CaptureFailureKind::secure_desktop;
  if (result == DXGI_ERROR_DEVICE_REMOVED || result == DXGI_ERROR_DEVICE_RESET)
    return CaptureFailureKind::device_removed;
  if (result == DXGI_ERROR_ACCESS_LOST) return CaptureFailureKind::access_lost;
  if (result == E_NOTIMPL || result == DXGI_ERROR_UNSUPPORTED)
    return CaptureFailureKind::unsupported;
  return CaptureFailureKind::backend_failed;
}
...
struct BackendState {
  ...
  std::optional<sources::MonitorTargetToken> target;
...
class CallbackGuard final {
 public:
  explicit CallbackGuard(std::shared_ptr<State> state) : state_(std::move(state)) {}
  ~CallbackGuard() {
...
void sendTerminal(const std::shared_ptr<BackendState>& state, CaptureFailure failure) {
...
class WgcMonitorCaptureBackendImpl final : public WgcMonitorCaptureBackend {
 public:
  explicit WgcMonitorCaptureBackendImpl(WgcMonitorCaptureOptions options)
    ...
    state_->diagnostics.d3d_debug_requested = options_.request_d3d_debug_layer;
  }
  ...
  BackendStartResult start(const sources::MonitorTargetToken& target, FrameCallback on_frame,
    ...
    if (!target.valid()) {
      return {false, CaptureFailure{"invalid_monitor_target", "native monitor target is empty"}};
    }
    try {
      ensureRoInitialized();
      {
      ...
      if (!GraphicsCaptureSession::IsSupported()) {
        return {false, CaptureFailure{"wgc_unsupported", "Windows Graphics Capture is unsupported",
                                      CaptureFailureKind::unsupported}};
      }
      if (state_->stop_requested.load()) {
        return {false,
                CaptureFailure{"capture_start_cancelled", "capture stopped during WGC startup"}};
      }

      bool debug_enabled = false;
      auto device = createWgcDevice(options_.request_d3d_debug_layer, debug_enabled);
      ...
      const auto frame_token =
          frame_pool.FrameArrived([weak, size](const Direct3D11CaptureFramePool& sender,
            ...
            try {
              ...
              if (content_size.Width != size.Width || content_size.Height != size.Height) {
                sendTerminal(state, {"wgc_output_changed", "Monitor dimensions changed",
                                     CaptureFailureKind::access_lost});
                return;
              }
              ...
              if (callback) {
                callback(BackendFrame{
                    ...
                    static_cast<std::uint32_t>(content_size.Height), FramePixelFormat::Bgra8,
                    makeWgcFrameResource(state->device, state->live_engine_resources,
              ...
              sendTerminal(state, {"wgc_frame_failed", hresultText(error.code()),
      ...
      const auto closed_token = item.Closed(
          [weak](const GraphicsCaptureItem&, const winrt::Windows::Foundation::IInspectable&) {
            if (const auto state = weak.lock()) {
              sendTerminal(state, {"source_removed", "captured monitor was removed",
      ...
      return {false, CaptureFailure{"wgc_start_failed", hresultText(error.code()),
                                    failureKind(error.code())}};
    } catch (const std::exception& error) {
  ...
  CaptureStopResult stop(std::chrono::steady_clock::time_point deadline) noexcept override {
    ...
    bool perform_cleanup = false;
    {
      std::lock_guard lock(state_->mutex);
      if (state_->stopped) {
        return state_->stop_failure ? CaptureStopResult{false, state_->stop_failure}
                                    : CaptureStopResult{};
      }
    ...
    if (perform_cleanup) {
      ...
      if (target) {
        ...
        if (!GetMonitorInfoW(reinterpret_cast<HMONITOR>(target->platformValue()), &info))
          cleanup([&] {
            detail::invalidateMonitorCaptureItem(target->platformValue(), target->cacheKey());
          });
      }
    }
    session = nullptr;
    ...
    if (!state_->callbacks_finished.wait_until(lock, deadline,
                                               [this] { return state_->active_callbacks == 0; })) {
      CaptureFailure failure{"wgc_callback_deadline_exceeded",
  ...
  void finalizeStop() noexcept override {
    ...
    state_->diagnostics.live_engine_objects = state_->live_engine_resources->load();
    state_->diagnostics.peak_engine_objects = state_->peak_engine_resources->load();
    if (state_->device) {
      {
    ...
    if (state_->device && state_->diagnostics.d3d_debug_enabled) {
      ...
      const HRESULT query = state_->device->owner->device()->QueryInterface(IID_PPV_ARGS(&debug));
      if (SUCCEEDED(query)) {
        state_->diagnostics.live_objects_hresult =
