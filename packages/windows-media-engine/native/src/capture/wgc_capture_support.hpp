...
winrt::Windows::Graphics::Capture::GraphicsCaptureItem acquireMonitorCaptureItem(
    std::uintptr_t platform_value, const std::string& stable_identity);
winrt::Windows::Graphics::Capture::GraphicsCaptureItem acquireWindowCaptureItem(
    std::uintptr_t platform_value, const std::string& stable_identity);
...
std::shared_ptr<FrameResource> makeWgcFrameResource(
    ...
    Microsoft::WRL::ComPtr<ID3D11Texture2D> texture, std::uint32_t width, std::uint32_t height);
