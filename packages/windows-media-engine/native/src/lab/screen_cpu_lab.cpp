...
using screen::ScreenSender;
using screen::ScreenSenderOptions;
...
struct ScreenEvidence {
  ...
  screen::ScreenSenderStats sender;
...
void appendSamples(std::vector<std::uint64_t>& destination,
                   const Values& source) {
  const auto available = screen::kMaximumScreenTimingSamples -
                         (std::min)(destination.size(),
                                    screen::kMaximumScreenTimingSamples);
...
ScreenEvidence runWindowCycle(SourceRegistry& registry,
  ...
  auto pipeline = std::make_shared<ScreenFramePipeline>();
  ScreenSenderOptions sender_options;
  sender_options.wait_for_unpublish = wait_until_unpublished;
  ScreenSender sender(room, pipeline, std::move(sender_options));
  std::function<void(ScreenSender&)> action;
...
ScreenEvidence runWindowRepeat(
  ...
  try {
    for (int cycle = 0; cycle < cycles; ++cycle) {
      ...
      std::cout << "publisher: screen track published" << std::endl;
      require(sender.waitForPublished(
                  static_cast<std::uint64_t>(cycle + 1) * 30ULL,
...
std::string runScreenCpuLab(const std::shared_ptr<livekit::Room>& room,
  ...
  auto after = currentResources();
  const auto settle_deadline = std::chrono::steady_clock::now() + 5s;
  while ((after.handles > before.handles + 8 ||
          after.threads > before.threads) &&
         std::chrono::steady_clock::now() < settle_deadline) {
    Sleep(10);
    after = currentResources();
  }
