...
inline constexpr ScreenVideoProfile kScreenProfile1080p60{1920, 1080, 60, 8'000'000};
inline constexpr ScreenVideoProfile kScreenProfile1440p30{2560, 1440, 30, 10'000'000};
inline constexpr ScreenVideoProfile kScreenProfile720p30{1280, 720, 30, 4'000'000};
...
struct GpuScreenConverterStats {
  ...
  std::uint64_t context_wait_total_us = 0;
  std::uint64_t context_hold_total_us = 0;
  std::uint64_t context_hold_max_us = 0;
...
class GpuNv12SlotLease final {
 ...
 private:
  GpuNv12SlotLease(std::shared_ptr<detail::GpuScreenConverterState> state, std::uint32_t slot);
...
// it flushes submission without mapping or waiting for GPU completion.
class GpuScreenConverter final {
 public:
  GpuScreenConverter(std::shared_ptr<capture::D3d11DeviceOwner> device_owner,
  ...
  [[nodiscard]] std::optional<GpuNv12SlotLease> convert(const capture::D3d11FrameView& frame,
