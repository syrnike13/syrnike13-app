...
This package owns the isolated lifecycle foundation for the replacement Windows native media engine. It contains the C++20 core, `media_probe.exe`, the thin `windows_media.node` binding, the Phase A LiveKit Room transport, probe-only Windows source discovery, and isolated WGC monitor capture. Audio, camera, encoding, preview, DXGI fallback, and compatibility code from v1 remain absent; capture is not wired to Engine, Electron, or LiveKit.
